...
cd /d %CYGWIN_SHELL%
::%CYGWIN_SHELL%\bash.exe --login -c 'cd %PROJECT_PATH%; make CONFIG=Release clean; make CONFIG=Release CWD=%PROJECT_PATH% -j%NUMCORE% all;'
%CYGWIN_SHELL%\bash.exe --login -c 'cd %PROJECT_PATH%; make CONFIG=Release clean; make CONFIG=Release CWD=%PROJECT_PATH% -j%NUMCORE% all;' > %PROJECT_PATH%/ReleaseBuild_Log.txt
::%CYGWIN_SHELL%\bash.exe --login -c "cd %PROJECT_PATH% && make CONFIG=Release clean && make CONFIG=Release CWD=%PROJECT_PATH% -j%NUMCORE% all" > %PROJECT_PATH%/ReleaseBuild_Log.txt
