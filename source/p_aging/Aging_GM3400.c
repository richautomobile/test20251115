...
#if (AGING_ENABLE==1U) && (PROJECT == GM_VCS_3400)
/* ==[ Macros ]============================================================= */
...
static uint16_t TimeCtrlCnt = 0U;
...
static void Aging_BoardPower(uint8_t OnOff);
static void Aging_ModulePower(uint8_t OnOff);
/*static void Aging_ModuleWakeUp(uint8_t OnOff);*/
static t_FUNC_RETURN Aging_FPDLink_Script(void);
static t_FUNC_RETURN Aging_TestProcess(void);
...
static t_FUNC_RETURN Aging_FPDLink_Script(void)
{
        ...
        //// Set up Variables
        //// *********************************************
        I2CMA_WriteB(serAddr, 0x70, desAddr0);
        I2CMA_WriteB(serAddr, 0x78, desAlias0);
        I2CMA_WriteB(serAddr, 0x88, 0x0);
        I2CMA_WriteB(serAddr, 0x71, 0x24);  /*walker add to bypass mcu i2c.*/
        I2CMA_WriteB(serAddr, 0x79, 0x24);  /*walker add to bypass mcu i2c.*/
        I2CMA_WriteB(serAddr, 0x89, 0x0);   /*walker add to bypass mcu i2c.*/
        ...
        //// *********************************************
        //// Both Ports enabled
        I2CMA_WriteB(desAlias0, 0x1, 0x1); //SW RESET Applied  //NG
...
static t_FUNC_RETURN Aging_TestProcess(void)
{
    ...
    switch(ItemSwitch)
    {
        case 0:  /*power on*/
            if(TimeCtrlCnt == 0U)
            {
                OPowerOnCnt = GmVCS_GetPwrCnt();
                Aging_ModulePower(1U);
                FPD983_SetPattern(White, White);
                GmVCS_SetDimm(8U, 0xFFFFU);
                getVal = GmVCS_GetPwrCnt();
        ...
        case 10: /*power off/on2*/
            if(TimeCtrlCnt == 0U)
            {
                Aging_ModulePower(0U);
            }

            if(TimeCtrlCnt >= 300) /*300esc*/
            {
                TimeCtrlCnt = 0U;
                ItemSwitch = 0x00U;
            }
...
t_FUNC_RETURN Aging_Init(void)
{
    Aging_BoardPower(1U);
    FPD983_SetI2cAddr(serAddr);
    FPD984_SetI2cAddr(desAddr0);
    
    Aging_ModulePower(1U);  /*Module On*/
...
t_FUNC_RETURN Aging_RoutineCall(void)
{
#if 0
...
t_FUNC_RETURN Project_Gpio_Init(void)
{
    ...
    Gpio_PinConfig(LED1, CY_GPIO_DM_STRONG, 0);
    Gpio_PinConfig(LED2, CY_GPIO_DM_STRONG, 0);
    Gpio_PinConfig(LED3, CY_GPIO_DM_STRONG, 0);
    Gpio_PinConfig(LED4, CY_GPIO_DM_STRONG, 0);
    Gpio_PinConfig(LED5, CY_GPIO_DM_STRONG, 0);
    Gpio_PinConfig(LED6, CY_GPIO_DM_STRONG, 0);

    Gpio_PinConfig(WAKE_SP, CY_GPIO_DM_STRONG, 0);

    Gpio_PinConfig(WAKE_FIDM, CY_GPIO_DM_STRONG, 0);
    Gpio_PinConfig(SW_Debug_Led, CY_GPIO_DM_STRONG, 0);

    Gpio_PinConfig(Breathe_LED, CY_GPIO_DM_STRONG, 0);
    Gpio_PinConfig(DBG_UART, CY_GPIO_DM_STRONG, 0);
